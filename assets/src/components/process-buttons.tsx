...
export const ProcessButtons = ({ process, server }: { process: ApiProcess; server: ApiSupervisorServer }) => {
  ...
  const running_buttons = (
    <div className='flex space-x-1'>
      <ActButton type={Type.Red} onClick={() => landingStore.stopProcess(server, process)}>
        <RiStopFill />
      </ActButton>
      <ActButton type={Type.Blue} onClick={() => landingStore.restartProcess(server, process)}>
        <TfiReload />
      </ActButton>
    </div>
  );
  ...
  const not_running_buttons = (
    <div className='flex space-x-1'>
      <ActButton type={Type.Green} onClick={() => landingStore.startProcess(server, process)}>
        <RiPlayFill />
      </ActButton>
    </div>
  );
