...
export const SupervisorBlockHeader = ({item}: { item: AllProcessInfoDTO }) => {
    ...
    const buttons = <div className="flex items-center space-x-1">
        <AddProgram/>
        <ActButton type={Type.Orange} onClick={() => landingStore.clearAllProcessLog(item.server.name)}><RiFileShredLine/></ActButton>
        <ActButton type={Type.Green} onClick={() => landingStore.startAll(item.server.name)}><RiPlayFill/></ActButton>
        <ActButton type={Type.Red} onClick={() => landingStore.stopAll(item.server.name)}><RiStopFill/></ActButton>
        <ActButton type={Type.Blue} onClick={() => landingStore.restartAll(item.server.name)}><TfiReload/></ActButton>
    </div>
